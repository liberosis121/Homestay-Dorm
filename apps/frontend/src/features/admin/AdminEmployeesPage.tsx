...
const MOCK_EMPLOYEES: Employee[] = [
  { id: 'NV-001', full_name: 'Trần Minh Khoa', email: 'khoa.tran@homestay.vn', phone: '090 111 2233', role: 'sale', branch: 'Quận 1', status: 'active', joinDate: '01/03/2023' },
  { id: 'NV-002', full_name: 'Nguyễn Thị Lan',  email: 'lan.nguyen@homestay.vn', phone: '091 222 3344', role: 'manager', branch: 'Quận 3', status: 'active', joinDate: '15/07/2022' },
  { id: 'NV-003', full_name: 'Lê Văn Đức',       email: 'duc.le@homestay.vn',    phone: '093 333 4455', role: 'accountant', branch: 'Quận 1', status: 'active', joinDate: '10/01/2024' },
  { id: 'NV-004', full_name: 'Phạm Thị Hoa',     email: 'hoa.pham@homestay.vn',  phone: '094 444 5566', role: 'sale', branch: 'Bình Thạnh', status: 'locked', joinDate: '05/09/2023' },
  { id: 'NV-005', full_name: 'Hoàng Admin',       email: 'admin@homestay.vn',     phone: '095 000 0000', role: 'admin', branch: 'Tất cả', status: 'active', joinDate: '01/01/2022' },
];
...
export default function AdminEmployeesPage() {
  ...
  const toggleLock = (id: string) => {
    setEmployees(prev => prev.map(e =>
      e.id === id ? { ...e, status: e.status === 'active' ? 'locked' : 'active' } : e
    ));
    if (selected?.id === id) setSelected(prev => prev ? { ...prev, status: prev.status === 'active' ? 'locked' : 'active' } : null);
  };
  ...
  const addEmployee = () => {
    ...
    setNewEmp({ full_name: '', email: '', phone: '', role: 'sale', branch: 'Quận 1' });
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up" style={{ fontFamily: 'Lexend, sans-serif' }}>
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: A.primary }}>Quản trị nhân viên</h1>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={e => { e.stopPropagation(); setSelected(emp); }}
                          className="p-1.5 rounded-full hover:opacity-70" style={{ color: A.accent }}>
                          <span className="material-symbols-outlined text-[18px]">edit</span>
                        </button>
                        <button onClick={e => { e.stopPropagation(); toggleLock(emp.id); }}
        ...
        <div className="fixed inset-0 z-50 flex justify-end"
          ...
          <div className="w-full max-w-[440px] h-full shadow-2xl flex flex-col animate-[slideInRight_0.3s_ease-out]"
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
              <div className="flex flex-col items-center text-center gap-3">
                <div className="w-20 h-20 rounded-full flex items-center justify-center text-3xl font-bold text-white"
                  style={{ background: A.primary }}>{selected.full_name.charAt(0)}</div>
                <div>
                  <h3 className="text-xl font-bold" style={{ color: A.primary }}>{selected.full_name}</h3>
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-1 ${ROLES[selected.role].cls}`}>
              ...
              <div className="p-4 rounded-lg" style={{ background: A.bg, border: `1px solid ${A.border}` }}>
                <h4 className="text-sm font-semibold mb-2" style={{ color: A.accent }}>Đổi vai trò</h4>
            ...
            <div className="px-6 py-4 flex gap-3" style={{ background: A.sidebar, borderTop: `1px solid ${A.border}` }}>
              <button className="flex-1 py-2.5 rounded-lg text-sm font-semibold text-white"
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="w-full max-w-md rounded-2xl shadow-2xl p-6 flex flex-col gap-4"
            style={{ background: A.surface }}>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1 uppercase" style={{ color: A.textMuted }}>Họ và tên</label>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1 uppercase" style={{ color: A.textMuted }}>Email</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold mb-1 uppercase" style={{ color: A.textMuted }}>Vai trò</label>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button onClick={addEmployee}
                ...
                style={{ background: A.primary }}>Thêm nhân viên</button>
            </div>
          </div>
        </div>
      )}
